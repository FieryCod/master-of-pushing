...
export class Main extends Phaser.State {
  ...
  private setupArena() {
    this.arena = new Phaser.Circle(
        this.world.centerX, this.world.centerY, this.world.height);
    // TODO: Replace temp graphic for arena
    this.graphics = this.game.add.graphics(0, 0);
    this.graphics.beginFill(TEMP_ARENA_COLOR, 1);
    this.graphics.drawCircle(
        this.world.centerX, this.world.centerY, this.arena.diameter);
    // --
  }
  private initPhysics(): void {
    ...
    this.players.forEach(
        p =>
            p.body.collides(playerCollisionGroups, this.playersCollideCallback),
        this);
  }
  private addPlayer(name: string) {
    let player = new Player(
        this.game, this.arena.x, this.arena.y, CONFIG.PLAYER_SPRITESHEET);
    player.name = name;
    player.scale.set(2);
    player.anchor.x = player.anchor.y = CONFIG.DEFAULT_ANCHOR;
    player.smoothed = false;
    player.animations.add(
        CONFIG.DEFAULT_ANIMATION_PLAYER, [0, 1, 2, 3, 4, 5], 10, true);
  ...
  private assignStartPositionsToPlayers() {
    ...
    for (let i = 0; i < positions; ++i) {
      ...
      player.body.x = player.startPosition.x = this.arena.x +
          (this.arena.radius - CONFIG.START_POS_EDGE_OFFSET) *
              Math.cos(angle * (Math.PI / 180));
      player.body.y = player.startPosition.y = this.arena.y +
          (this.arena.radius - CONFIG.START_POS_EDGE_OFFSET) *
  ...
  private nextRound() {
    ...
    if (this.players.countDead()) this.players.callAll('revive', null);
  }
  ...
  private showGameResults() {
    ...
    let text = this.game.add.text(
        this.game.world.centerX, 300, this.winningText, CONFIG.TEXT_OPTIONS);
    text.anchor.set(CONFIG.DEFAULT_ANCHOR);
    text.alpha = 0;
    rect.alpha = 0;
    this.game.add.tween(text).to(
        {alpha: 1}, duration, Phaser.Easing.Linear.None, true, delayTween, 0,
        false);
