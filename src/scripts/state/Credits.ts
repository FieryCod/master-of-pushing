...
export class Credits extends Phaser.State {
  ...
  private addDev(position: string, devName: string, curr: number) {
    let text1 = this.game.add.text(
        this.world.centerX, 300 + (150 * curr), `Position: ${position}`,
        CONFIG.SMALLER_TEXT_OPTIONS);
    let text2 = this.game.add.text(
        this.world.centerX, 350 + (150 * curr), `Name: ${devName}`,
        CONFIG.SMALLER_TEXT_OPTIONS);
